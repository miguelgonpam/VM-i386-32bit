...
/** 
 *  Bild_elf_main function. Executes the i386 emulator without graphic interface.
 *
 ...
 *
 */
int blind_elf_main(int argc, char *argv[], char *envp[]){
...
/** 
 *  Bild_raw_main function. Executes the i386 emulator without graphic interface.
 *
 *  @param argc number of arguments (argv size).
 *  @param argv array containing pointer to args.
 *  @param envp array containing pointer to environment variables.
...
int blind_raw_main(int argc, char *argv[], char *envp[]){
   ...
   while (true){
      ...
      /* Disassemble the current instruction. Bad alignment could have happened. */
      if(!cs_disasm(handle, &mem[eip], size-eip, eip, 1, &ins)) // If number of disasm instructions is 0.
         goto exit;
...
/** 
 *  Interface_raw_main function. Executes the i386 emulator with graphic interface.
 *
 *  @param argc number of arguments (argv size).
 *  @param argv array containing pointer to args.
 *  @param envp array containing pointer to environment variables.
 ...
 *
 */
int interface_raw_main(int argc, char *argv[], char *envp[]){
   ...
   while ('q' != ch){
      ...
      for (size_t i = 0; i < MIN(rows, count -scr_c+1); i++) {
         ...
         uint32_t addr = insn[index].address;
         if (addr == eip){
            eip_ind = i;
         }
         snprintf(lineas[i*2], ADDR_TXT_S-1, "0x%08x", addr);
         snprintf(lineas[i*2+1], MAX_STR, "%.10s %.50s", insn[index].mnemonic, insn[index].op_str);
         exited = i;
      }
...
int main(int argc, char *argv[], char *envp[]){
   ...
   if(argc >= 2 && strcmp(argv[1], "-h") == 0){
      fprintf(stdout, "Usage: %s <mode> <file>\n-h prints manual page. Shows all the commands\n-ie runs the emulator with graphic interface using an ELF file\n-ne runs the emulator using an elf file in normal mode, as if the program was executed from the terminal\n-ir runs the emulator with graphic interface using a raw file\n-nr runs the emulator using a raw file in normal mode, as if the program was executed from the terminal\n", argv[0]);
      return 0;
   }else if (argc < 3) {
      usage:

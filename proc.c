...
void instr_add_rm8_r8(unsigned char *code, unsigned int *stack){
   ...
   unsigned char  * rm8 = (unsigned char *)regs[(modrm & 0b00000111)];
   unsigned char  * r8 = (unsigned char *)regs[(modrm & 0b00111000)];
   *rm8 += *r8;
   //flags
}
...
void instr_adc_ib_al(unsigned char *code, unsigned int *stack){
   ...
   unsigned char * f = (unsigned char *)&eflags;
   f+=3;
   *al= *al+b+( *f & 0b00000001);
}
...
void instr_arpl(unsigned char *code, unsigned int *stack){
   ...
   unsigned char * f = (unsigned char *)&eflags; f+=3;
   v1 = *rm16 & 0x3;
   v2 = *r16 & 0x3;
   if (v1 < v2){
      *f|=0b00001000;
 ...
 * OPCODE 0x80
 */
void instr_adc_reg8_imm8(unsigned char *code, unsigned int *stack){
   unsigned short int v = *(code+1) ; //byte ModR/M
   unsigned short int * f = (unsigned short int *)&eflags;
   f+=3;
   v&=0b00000111;
   unsigned char * reg = (unsigned char *) regs[v];
   *reg = *reg + *(code+2) + (*f & 0b00000001);
}

/**
 * OPCODE 0x81
 */
void instr_adc_reg16_32_imm16_32(unsigned char *code, unsigned int *stack){
   ...
   *reg = *reg + v + ( *f & 0b00000001);
}

/**
 * OPCODE 0x83
 */
void instr_adc_reg16_32_imm8(unsigned char *code, unsigned int *stack){
   unsigned short int v = *(code+1) ; //byte ModR/M
   unsigned short int * f = (unsigned short int *)&eflags;
   f+=3;
   v&=0b00000111;
...
void fillInstr(InstrFunc * i){
   ...
   i[0x80]=instr_adc_reg8_imm8;
   i[0x81]=instr_adc_reg16_32_imm16_32;
}
...
int main(){
   ...
   eax = 0x000A0000;
   printf("eax : 0x%8x\n", eax);
   unsigned char code[6] ={ 0x81, 0xD0, 0xF7, 0x33, 0xFF, 0xE0};
   instr_adc_reg16_32_imm16_32(code, stack);
   printf("eax : 0x%8x\n", eax);
